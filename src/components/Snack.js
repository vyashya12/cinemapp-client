...
function Snack({data, getFood}) {
    ...
    let deleteHandler = (id, handleClose) => {
        fetch(`${process.env.REACT_APP_API_URL}snacks/`+id, {
            method: "DELETE",
            headers: {
    ...
    let onSubmitHandler =(e) => {
        ...
        fetch(`${process.env.REACT_APP_API_URL}snacks/${edit.id}`, {
            method: "PUT",
            headers: {
                "x-auth-token": localStorage.getItem('token')
            },
    ...
    let bookHandler = (e) => {
        e.preventDefault()
        fetch(`${process.env.REACT_APP_API_URL}booking/${user.sub}`, {
            method: "POST",
            headers: {
                "x-auth-token": localStorage.getItem('token'),
    ...
    return(
                <Col>
                    <Card style={styles}>
                        <Image style={imageStyle} src={`${process.env.REACT_APP_API_URL}${data.image.split('/')[2]}`} alt={data.name} />
                        <Card.Content>
